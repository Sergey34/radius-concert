...
import seko0716.radius.concert.admin.services.parsers.kassir.KassirCityParser.Companion.ATTR_HREF
import seko0716.radius.concert.admin.services.parsers.kassir.KassirCityParser.Companion.CSS_QUERY_CITY
import seko0716.radius.concert.admin.services.parsers.kassir.KassirPage.Companion.INVALID_KASSIR_PAGE
...
@Component
class KassirEventParser @Autowired constructor(
    ...
    private suspend fun createEvent(event: Element, city: City) = attempt({
        val content = event.select(JSON_DATA).attr(
            JSON_DATA_ITEM
        )
